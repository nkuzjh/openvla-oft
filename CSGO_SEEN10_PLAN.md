...
本文记录已实现的 legacy / aligned v2 的设计、配置依据、实现边界和验收证据。环境、手动运行命令、输出目录和正式结果见 [CSGO_SEEN10.md](CSGO_SEEN10.md)。记录更新于 2026-09-25，代码工作基于 `main@41c7227ce3bdfe48acfef7a6104958836201b0f7` 及工作区中的 aligned 变更；文档整理后已补充跨服务器路径适配，见第 7.3 节；不重新运行模型或正式实验。
...
`load_config` 读取 YAML；通用 `_config_value` 顺序为顶层字段 → `train.<key>` → `model.<key>` → 代码默认。地图、路径等字段有各自解析函数；数据路径另受 `CSGO_DATA_ROOT > DATA_ROOT > YAML` 控制。共享 evaluator 和 Python 可由 `SHARED_EVAL_DIR`、`CSGO_EVAL_PYTHON`（兼容 `UNILIP_PYTHON`）覆盖。CLI 提供 config、seed、smoke、resume/checkpoint，以及 data-root、eval-root、unilip-python、model-path 路径覆盖和只读 print-paths，不是任意 YAML key 覆盖器。路径 CLI 优先于环境变量，配置与相对路径统一以项目根目录解析；evaluator Python 默认使用通用评测器目录的 .venv；显式 Python 路径不校验、不回退，详见运行说明第 3 节。
...
- [paths.py](csgo_seen10/paths.py) 统一 CLI→环境变量→YAML→默认路径，文件相对 checkout 根解析，Python symlink 保留 venv 身份；仅旧服务器内置数据/evaluator 默认在缺失时允许回退，错误自定义路径不隐式替换。评测解释器的选择无存在性检查或回退。
...
- 2026-09-25 起 evaluator 默认使用 `<shared_eval_dir>/.venv/bin/python`，由用户通过评测器 `setup_env.sh` 单独安装。两份 YAML 设 `unilip_python: null`，保留 CLI/env 显式覆盖；OpenVLA eval 不增加安装、验证、修复或回退逻辑，缺失解释器直接报错。原服务器历史指标不因解释器改变而重写。统一依赖与实际环境版本由评测器项目维护。
...
2026-09-25 本机统一评测环境已独立安装：Python 3.11.16、torch 2.7.1+cpu、torchvision 0.22.1+cpu；四个评测模块导入及 `pip check` 通过，没有加载指标模型权重或执行评测。版本证据见 [install-manifest.json](../csgo_benchmark_v2_eval_general/.venv/install-manifest.json)，完整依赖见同目录 `pip-freeze.txt`。OpenVLA 路径测试增至 5 项并通过，覆盖解释器选择不检查存在性、不回退；两份 YAML 仅更改评测解释器字段，训练配方不变。另一台服务器仍由用户同步代码后独立运行安装脚本。
...
- 路径适配和统一环境安装不修改 UniLIP、数据或评测指标实现。2026-09-25 增加通用评测器环境安装入口/依赖及 OpenVLA 默认解释器选择；仅安装独立评测环境并做导入/路径检查，未启动训练、推理或评测。后续正式运行仍由用户手动执行。
